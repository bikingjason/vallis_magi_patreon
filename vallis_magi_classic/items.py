...
class ItemManager:
    ...
    # ) -> ItemDefinition:
    #     """
    #     Choose one item definition from an isa group.

    #     Rogue-style algorithm:

    #     - Sort items by priority.
    #     - Keep a running cumulative probability.
    #     - For each item:
    #         - roll d100
    #         - if roll <= cumulative probability, choose that item
    #         - otherwise continue
    #     - The final item should have cumulative probability 100.

    #     Example:
    #         item = loader.choose_item_definition("is_potion")
    #         item = loader.choose_item_definition("is_scroll", verbose=True)
    #     """
    ...
    #     if not item_defs:
    #         raise ValueError(f"No item definitions found for {isa_name!r}")
    ...
    #         print("-" * (len("Choosing item from ") + len(isa_name)))

    #     for item_def in item_defs:
    #         cumulative_probability += item_def.probability
    ...
    #             print(
    #                 f"roll={roll:>3}  "
    #                 f"priority={item_def.priority:>3}  "
    #                 f"probability={item_def.probability:>3}  "
    #                 f"cumulative={cumulative_probability:>3}  "
    #                 f"item={item_def.id}"
    #             )

    #         if roll <= cumulative_probability:
    #             if verbose:
    #                 print(f"Selected: {item_def.id} - {item_def.name}")
    #             return item_def

    #     # This should not happen if the final cumulative probability is 100.
    #     raise RuntimeError(f"Failed to choose item from {isa_name!r}. Final cumulative probability was {cumulative_probability}, expected 100.")
    ...
    #     roll = rng.randrange(100)  # 0..99, matching Rogue rnd(100)
    #     cumulative_probability = 0
    ...
    #         if verbose:
    #             print(
    #                 f"priority={item_def.priority:>3}  probability={item_def.probability:>3}  cumulative={cumulative_probability:>3}  item={item_def.id}"
    #             )
    ...
    #     raise RuntimeError(
    #         f"Failed to choose item from {isa_name!r}. Roll was {roll}, final cumulative probability was {cumulative_probability}, expected 100."
    #     )
